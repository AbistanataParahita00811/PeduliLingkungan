...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-base font-semibold text-forest">Tambah Event</h1>
    </div>

    <form action="{{ route('admin.events.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-4 space-y-4">
        @csrf
        ...
        <div class="grid md:grid-cols-[1.3fr_1fr] gap-4">
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" maxlength="200" class="w-full text-xs border rounded-md px-3 py-2">{{ old('description') }}</textarea>
                </div>
                <div>
                ...
                <div class="grid md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="event_date" value="{{ old('event_date') }}" data-datepicker class="w-full text-xs border rounded-md px-3 py-2" required>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1">Jam Event</label>
                        <input type="text" name="event_time" value="{{ old('event_time') }}" data-timepicker class="w-full text-xs border rounded-md px-3 py-2">
                    </div>
                </div>
                ...
                <div class="grid md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="location" value="{{ old('location') }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold text-gray-700 mb-1">Kategori</label>
                        <input type="text" name="category" value="{{ old('category', 'Tanam Pohon') }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">Tags (pisahkan dengan koma)</label>
                    <input type="text" name="tags_input" value="{{ old('tags_input') }}" class="w-full text-xs border rounded-md px-3 py-2" placeholder="tanam pohon, edukasi, gratis">
                </div>
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    ...
                    <label class="inline-flex items-center gap-2 text-xs text-gray-700">
                        <input type="checkbox" name="show_in_navbar" value="1" class="rounded border-gray-300" @checked(old('show_in_navbar'))>
                        Tampilkan di Navbar
                    </label>
                </div>

                <div class="card-admin mt-6" x-data="{ hasPopup: {{ old('has_popup', false) ? 'true' : 'false' }} }">
