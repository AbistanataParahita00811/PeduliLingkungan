...
@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-base font-semibold text-forest">Edit Foto Galeri</h1>
    </div>
    ...
    <form
        ...
        x-data="{ previewUrl: '{{ asset('storage/'.$gallery->image) }}' }"
    >
        @csrf
        @method('PUT')
        ...
        <div class="grid md:grid-cols-[1.3fr_1fr] gap-4">
            <div class="space-y-3">
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title', $gallery->title) }}" class="w-full text-xs border rounded-md px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">Caption (opsional)</label>
                    <textarea name="caption" rows="3" class="w-full text-xs border rounded-md px-3 py-2">{{ old('caption', $gallery->caption) }}</textarea>
                </div>
                <div class="grid md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="event_id" class="w-full text-xs border rounded-md px-3 py-2">
                            ...
                            @foreach($events as $event)
                                <option value="{{ $event->id }}" @selected(old('event_id', $gallery->event_id) == $event->id)>
                                    {{ $event->title }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <input
                            type="date"
                            name="activity_date"
                            value="{{ old('activity_date', optional($gallery->activity_date)->format('Y-m-d')) }}"
                ...
                <div class="grid md:grid-cols-3 gap-3">
                    <div class="flex items-center gap-2">
                        <input type="checkbox" name="is_featured" value="1" class="rounded border-gray-300" @checked(old('is_featured', $gallery->is_featured))>
                    ...
                    <div>
                        ...
                        <input type="number" name="order_index" value="{{ old('order_index', $gallery->order_index) }}" class="w-full text-xs border rounded-md px-3 py-2">
                    </div>
                </div>
            </div>
            <div>
