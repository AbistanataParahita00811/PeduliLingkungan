...
@section('page_title', 'Tambah Testimonial')

@section('content')
    <div class="flex items-center justify-between mb-4">
        <h1 class="text-base font-semibold text-forest">Tambah Testimonial</h1>
    </div>

    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-2xl shadow-sm p-4 space-y-4">
        @csrf
        ...
        <div class="grid md:grid-cols-[1.3fr_1fr] gap-4">
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <input type="text" name="role" value="{{ old('role') }}" class="w-full text-xs border rounded-md px-3 py-2">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-700 mb-1">Quote</label>
                    <textarea name="quote" rows="4" class="w-full text-xs border rounded-md px-3 py-2" required>{{ old('quote') }}</textarea>
                </div>
